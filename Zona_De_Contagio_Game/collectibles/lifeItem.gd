...
func _process(delta):
	...
	move_object_towards_player()

func _on_player_nier_entered(body):
	if body.is_in_group("Player"):
		animate = false
		_player = body
		move_object_towards_player()
		
func move_object_towards_player():
	if(_player == null):
		return
	var direction = _player.global_position - self.global_position
	direction = direction.normalized()
	self.global_position += direction * 0.5

func _on_area_2d_body_entered(body):
	if body.is_in_group("Player"):
		body.increase_health(qtdRecupera)
		queue_free()
